...
$(document).ready(function() {
  ...
  $.ajax({
    ...
    success: function(data) {
      for (var element in data) {
        usercount++;
        if (usercount > 4) break; // limit to 4 users only
        userlistdata += '<li><img src="dist/img/generic-user.png" alt="User Image"><div class="users-list-name">' + data[element].firstname + ' ' + data[element].lastname + '</div><span class="users-list-date">' + moment(data[element].lastAvailable, "YYYY-MM-DD, h:mm:ss").fromNow() + '</span></li>';
      }
