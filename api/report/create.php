...
if (($report->replacement_registryId AND $report->faulty_registryId) AND ($report->replacement_registryId == $report->faulty_registryId)){
    ...
    if($report->create()){
        $output_arr=array(
            ...
            "assignee_userId" => $report->assignee_userId,
            "faulty_registryId" => $report->faulty_registryId,
            "replacement_registryId" => $report->replacement_registryId,
            "dateRequested" => $report->dateRequested,
            "dateLeaving" => $report->dateLeaving,
            "dateDispatched" => $report->dateDispatched,
            "dateReturned" => $report->dateReturned,
            "AWB" => $report->AWB,
            "AWBreturn" => $report->AWBreturn,
            "RMA" => $report->RMA
        );
    }
    else{
